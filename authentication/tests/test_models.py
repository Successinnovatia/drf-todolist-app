...
class TestModel(APITestCase):
    ...
    def test_creates_user(self):
        user=User.objects.create_user('testuser', 'testuser@email.com')
        self.assertIsInstance(user,User)
        self.assertFalse(user.is_staff)
        self.assertEqual(user.email, 'testuser@email.com')

    def test_creates_super_user(self):
        user=User.objects.create_superuser('testsuperuser', 'superuser@email.com' )
        self.assertIsInstance(user, User)
        self.assertTrue(user.is_staff)
        self.assertEqual(user.email, 'superuser@email.com')

    def test_raises_error_when_no_username_is_supplied(self):
        self.assertRaises(ValueError, User.objects.create_user,username="", email='testuser@email.com', password='Password123!@')
    ...
    def test_raises_error_when_no_email_is_supplied(self):
        self.assertRaises(ValueError, User.objects.create_user,username='testuser', email='', password='Password123!@')

    def test_raises_error_with_message_when_no_email_is_supplied(self):
        
        with self.assertRaisesMessage(ValueError, 'The given email must be set' ):
            User.objects.create_user(username='tetuser', email='', password='Password123!@')
